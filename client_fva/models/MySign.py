...
class MySignModel(QSqlQueryModel):
    ...
    def get_all(self):
        sql = 'select document_name, signed_document_path, transaction_status, transaction_text from mysigns ' \
    ...
    def filter(self, text):
        sql = 'select document_name, signed_document_path, transaction_status, transaction_text from mysigns where ' \
              'document_name like ? and  userid = ? order by id DESC'
        query = QSqlQuery(self.db)
        query.prepare(sql)
