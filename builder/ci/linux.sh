...
cp /builder/debian/copyright /tmp/
cp /builder/debian/postinst /tmp/
...
mv /data/build/${PACKAGE}_${VERSION}_${ARCH}.deb $OLD_PATH/
mv /data/build/${PACKAGE}-${VERSION}-2.${RPM_ARCH}.rpm  $OLD_PATH/
mv /data/build/lsb-${PACKAGE}-${VERSION}-2.${RPM_ARCH}.rpm $OLD_PATH/
mv /data/build/${PACKAGE}-${VERSION}.tgz $OLD_PATH/
